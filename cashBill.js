...
function init() {
    if (window.localStorage.getItem(cashBill_data) == null) {
        let item = new Product(1,"Black Caffe",0,16000)
        products.push(item)
        ...
        return `
            <tr id="tr_${can.productId}">
                ...
                <td class="btnover">
                    ...
                    <button class="btn" onclick="cancelProduct(${can.productId})">Cancel</button>
                </td>
            <tr>
        `
    })
...
function removeProduct(productId){
    let comfirmed = window.confirm("Bạn muốn xóa sản phẩm này?") // trả về bolean
